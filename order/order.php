...
<div class="orderBackground">
    ...
    <div class="moreNewsContentAdd">
        ...
        if (isset($_SESSION['logged_user'])) {
            ...
            $orders = R::find('order', "WHERE iduser = ? ORDER BY 'id IS NOT NULL'DESC", array($_SESSION['logged_user']->id));
            if (!isset($orders->array)) {
                foreach ($orders as $order) {
                    $product = R::findOne('product', "id = ? ", array(2));
                                    ...
                                    <table>                                    
                                        ...
                                        <tr>
                                            ...
                                            <td rowspan='2'>
                                                ...
                                                <input type='button' class='buttonOrder' value='Удалить позицию'> 
                                                
                                                </div>
                                             </td>
                                        </tr>
                                        <tr>
                                            <td> <img class='imgTable' src ='".$product->photo."'> </td>
                                            <td> <b>$product->nameproduct,</b><br> $product->description</td>
                                            <td> ";
                                            $price = 0;
                                            if($product->pricepershare >0 && $product->pricepershare < $product->price)
                                            {$price = $product->pricepershare;}
                                            else
                                            {$price = $product->price;}
                                            echo "$price</td>

                                            <td><input type='number' max='$product->productcount' value='$order->countproduct'/></td>                                            
                                        </tr>                                       
                                    </table>                                    
                              </div>                
					</div>";
